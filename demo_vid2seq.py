...
parser.add_argument("--asr_dir", type=str, required=True, help="Directory containing ASR files")
parser.add_argument("--output_dir", type=str, required=True, help="Directory to save dense caption output files")
...
for video_file in os.listdir(args.video_dir):
    if video_file.endswith(".mp4"):  # Adjust the file extension if needed
        ...
        asr_file = os.path.join(args.asr_dir, f"{filename}_asr.pkl")
        
        if not os.path.exists(asr_file):
            print(f"Skipping {video_file} as no corresponding ASR file found.")
            continue
        
        # Code that needs to be run for each video
        ...
        print("load ASR")
        segments = pickle.load(open(asr_file, 'rb'))["segments"]
        texts, starts, ends = [], [], []
        ...
        if not sub['text']:
            ...
            time_input_tokens = [torch.LongTensor([time_tokenize(st, duration, args.num_bins, len(tokenizer) - args.num_bins), time_tokenize(ed, duration, args.num_bins, len(tokenizer) - args.num_bins)]) for st, ed in zip(sub['start'], sub['end'])]
            text_input_tokens = [tokenizer(x, add_special_tokens=False, max_length=args.max_input_tokens, padding="do_not_pad", truncation=True, return_tensors="pt",)['input_ids'][0] for x in sub['text']]
        ...
        with torch.no_grad():
            output = model.generate(video=video, input_tokenized=input_tokenized, use_nucleus_sampling=args.num_beams == 0, num_beams=args.num_beams, max_length=args.max_output_tokens, min_length=1, top_p=args.top_p, repetition_penalty=args.repetition_penalty, length_penalty=args.length_penalty, num_captions=1, temperature=1)
        ...
        # Save the dense caption output to the specified output directory
        output_file = os.path.join(args.output_dir, f"{filename}-caption.txt")
